...
public class Developer
{
    ...
    public static void readAll(List<Developer> list)
    {
        list.clear();       // Clear the target list first.

        /* Create a new prepared statement object with the desired SQL query. */
        PreparedStatement statement = Application.database.newStatement("SELECT developerId, name FROM categories ORDER BY developerId"); 
        ...
        if (statement != null)      // Assuming the statement correctly initated...
        {
            ...
            if (results != null)        // If some results are returned from the query...
            {
                try {								// ...add each one to the list.
                    while (results.next()) {        			                           
                        list.add( new Developer(results.getInt("developerId"), results.getString("name")));
                    }
                }
